...
async fn test_mine() {
    ...
    // Submit register tx
    let proof_pda = Pubkey::find_program_address(&[PROOF, payer.pubkey().as_ref()], &spam::id());
    let ix = spam::instruction::register(payer.pubkey());
    ...
    assert_eq!(proof_account.owner, spam::id());
    ...
    // Submit mine tx
    let ix = spam::instruction::mine(payer.pubkey(), BUS_ADDRESSES[0], next_hash.into(), nonce);
    let tx = Transaction::new_signed_with_payer(&[ix], Some(&payer.pubkey()), &[&payer], blockhash);
    let res = banks.process_transaction(tx).await;
    assert!(res.is_ok());
    ...
    let beneficiary_address = get_associated_token_address(&payer.pubkey(), &spam::MINT_ADDRESS);
    ...
    let ix = spam::instruction::claim(payer.pubkey(), beneficiary_address, amount);
    ...
    assert_eq!(beneficiary.mint, spam::MINT_ADDRESS);
...
async fn test_mine_alt_proof() {
    ...
    let proof_alt_pda =
        Pubkey::find_program_address(&[PROOF, payer_alt.pubkey().as_ref()], &spam::id());
    let ix_alt = spam::instruction::register(payer_alt.pubkey());
    let tx = Transaction::new_signed_with_payer(
        &[ix_alt],
...
async fn test_claim_too_large() {
    ...
    // Submit claim tx
    let beneficiary = get_associated_token_address(&payer.pubkey(), &spam::MINT_ADDRESS);
    let token_ix = create_associated_token_account(
        &payer.pubkey(),
        &payer.pubkey(),
        &spam::MINT_ADDRESS,
    ...
    let ix = spam::instruction::claim(payer.pubkey(), beneficiary, 1);
    let tx = Transaction::new_signed_with_payer(
        &[token_ix, ix],
        Some(&payer.pubkey()),
        &[&payer],
...
async fn test_claim_other_proof() {
    ...
    // Submit claim tx
    let beneficiary = get_associated_token_address(&alt_payer.pubkey(), &spam::MINT_ADDRESS);
    let token_ix = create_associated_token_account(
        &alt_payer.pubkey(),
        &alt_payer.pubkey(),
        &spam::MINT_ADDRESS,
    ...
    let mut ix = spam::instruction::claim(payer.pubkey(), beneficiary, 0);
...
async fn test_mine_not_enough_accounts() {
    ...
    // Submit mine tx
    let mut ix = spam::instruction::mine(payer.pubkey(), BUS_ADDRESSES[0], next_hash.into(), nonce);
...
async fn test_mine_fail_bad_data() {
    ...
    let proof_address = Pubkey::find_program_address(&[PROOF, signer.as_ref()], &spam::id()).0;
