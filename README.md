...
Spam is designed to protect holders from runaway supply inflation. Regardless of how many miners are active in the world, supply growth is strictly bounded to a rate of `0 ≤ R ≤ 2 SPAM/min`. In other words, linear. The mining reward rate – amount paid out to miners per valid solution – is dynamically adjusted every 60 seconds to maintain an average supply growth of `1 SPAM/min`. This level was chosen for its straightforward simplicity, scale agnosticism, and for striking a balance between the extremes of exponential inflation on one hand and stagnant deflation on the other.
...
- [`Idl`](idl/spam.json) – Interface for clients, explorers, and programs.
...
## Instructions
- [`Initialize`](src/processor/initialize.rs) – Initializes the Spam program, creating the bus, mint, and treasury accounts.
